...
static struct cpufreq_gabriel_tunables *tuned_parameters[NR_CPUS] = {NULL, };
...
static int cpufreq_gabriel_speedch_task(void *data)
{
	unsigned int cpu;
	cpumask_t tmp_mask;
	unsigned long flags;
...
static ssize_t show_align_windows(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%u\n", tunables->align_windows);
}

static ssize_t store_align_windows(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_timer_rate_idle_freq(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%lu\n", tunables->timer_rate_idle_freq);
}

static ssize_t store_timer_rate_idle_freq(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int ret;
	long unsigned int val;
...
/* freq_responsiveness */
static ssize_t show_freq_responsiveness(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->freq_responsiveness);
}

static ssize_t store_freq_responsiveness(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int input;
	int ret;
...
static ssize_t show_pump_inc_step_at_min_freq(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->pump_inc_step_at_min_freq);
}

static ssize_t store_pump_inc_step_at_min_freq(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int input;
	int ret;
...
/* pump_inc_step */
static ssize_t show_pump_inc_step(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->pump_inc_step);
}

static ssize_t store_pump_inc_step(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int input;
	int ret;
...
static ssize_t show_pump_dec_step_at_min_freq(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->pump_dec_step_at_min_freq);
}

static ssize_t store_pump_dec_step_at_min_freq(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int input;
	int ret;
...
/* pump_dec_step */
static ssize_t show_pump_dec_step(struct cpufreq_gabriel_tunables *tunables,
		char *buf)
{
	return sprintf(buf, "%d\n", tunables->pump_dec_step);
}

static ssize_t store_pump_dec_step(struct cpufreq_gabriel_tunables *tunables,
		const char *buf, size_t count)
{
	int input;
	int ret;
...
static ssize_t show_index_update(struct cpufreq_gabriel_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->index_update);
}

static ssize_t store_index_update(struct cpufreq_gabriel_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
unsigned int cpufreq_gabriel_get_hispeed_freq(int cpu)
{
	struct cpufreq_gabriel_cpuinfo *pcpu =
			&per_cpu(cpuinfo, cpu);
	struct cpufreq_gabriel_tunables *tunables;
...
static int cpufreq_gabriel_cluster1_min_qos_handler(struct notifier_block *b,
...
static struct notifier_block cpufreq_gabriel_cluster1_min_qos_notifier = {
	.notifier_call = cpufreq_gabriel_cluster1_min_qos_handler,
};

static int cpufreq_gabriel_cluster1_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_gabriel_cpuinfo *pcpu;
	struct cpufreq_gabriel_tunables *tunables;
...
static struct notifier_block cpufreq_gabriel_cluster1_max_qos_notifier = {
	.notifier_call = cpufreq_gabriel_cluster1_max_qos_handler,
};

#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
static int cpufreq_gabriel_cluster0_min_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_gabriel_cpuinfo *pcpu;
	struct cpufreq_gabriel_tunables *tunables;
...
static struct notifier_block cpufreq_gabriel_cluster0_min_qos_notifier = {
	.notifier_call = cpufreq_gabriel_cluster0_min_qos_handler,
};

static int cpufreq_gabriel_cluster0_max_qos_handler(struct notifier_block *b,
						unsigned long val, void *v)
{
	struct cpufreq_gabriel_cpuinfo *pcpu;
	struct cpufreq_gabriel_tunables *tunables;
...
static struct notifier_block cpufreq_gabriel_cluster0_max_qos_notifier = {
	.notifier_call = cpufreq_gabriel_cluster0_max_qos_handler,
};
#endif
#endif
...
static int __init cpufreq_gabriel_init(void)
{
	...
	speedch_task =
		kthread_create(cpufreq_gabriel_speedch_task, NULL,
				"cfgabriel");
	if (IS_ERR(speedch_task))
		return PTR_ERR(speedch_task);

	kthread_bind(speedch_task, 0);

#ifdef CONFIG_ARCH_EXYNOS
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MIN, &cpufreq_gabriel_cluster1_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER1_FREQ_MAX, &cpufreq_gabriel_cluster1_max_qos_notifier);
#ifdef CONFIG_ARM_EXYNOS_MP_CPUFREQ
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MIN, &cpufreq_gabriel_cluster0_min_qos_notifier);
	pm_qos_add_notifier(PM_QOS_CLUSTER0_FREQ_MAX, &cpufreq_gabriel_cluster0_max_qos_notifier);
#endif
#endif
