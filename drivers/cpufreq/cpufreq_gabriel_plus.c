...
#define DEFAULT_BUMP_FREQ_WEIGHT_DIVISION 186
#define DEFAULT_POWER_BUMP_FREQ_WEIGHT_DIVISION 250
...
struct cpufreq_gabriel_plus_tunables {
	...
	bool go_hispeed_load_adt_freq_calc;
	...
	unsigned long power_max_local_load_division;
	...
	unsigned int *policy;
	/*
	 * reduce frequency in high CPU load
	 */
	bool power_save_cpu_load;
	bool power_save_max_local_load;
	/*
...
static void cpufreq_gabriel_plus_timer(unsigned long data)
{
	...
	unsigned int power_max_local_load_division = tunables->power_max_local_load_division;
	unsigned int bump_freq_weight = tunables->bump_freq_weight;
		...
		if (cpu_load >= tunables->go_hispeed_load) {
			if (pcpu->policy->cur < this_hispeed_freq &&
				...
				if (tunables->go_hispeed_load_adt_freq_calc) {
					...
					if (tunables->power_save_max_local_load)
						new_freq = this_hispeed_freq * tunables->bump_freq_weight / tunables->power_bump_freq_weight_division;
					else
						new_freq = this_hispeed_freq * tunables->bump_freq_weight / tunables->bump_freq_weight_division;
				}
			...
			if (tunables->power_save_max_local_load) {
				if (new_freq > tunables->freq_calc_thresh &&
					cpu_load < tunables->max_local_load / tunables->power_max_local_load_division) {
					new_freq = pcpu->policy->cur * tunables->bump_freq_weight / tunables->power_bump_freq_weight_division;
					if (tunables->power_save_cpu_load) {
						new_freq = pcpu->policy->max * cpu_load / tunables->power_cpu_load_max_division;
					} else {
						new_freq = pcpu->policy->max * cpu_load / tunables->cpu_load_max_division;
					}
				}
			} else {
				if (new_freq > tunables->freq_calc_thresh &&
					cpu_load < tunables->max_local_load / tunables->max_local_load_division) {
					new_freq = pcpu->policy->cur * tunables->bump_freq_weight / tunables->bump_freq_weight_division;
					if (tunables->power_save_cpu_load) {
						new_freq = pcpu->policy->max * cpu_load / tunables->power_cpu_load_max_division;
					} else {
						new_freq = pcpu->policy->max * cpu_load / tunables->cpu_load_max_division;
...
static ssize_t show_go_lowspeed_load(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->go_lowspeed_load);
}

static ssize_t store_go_lowspeed_load(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_power_max_local_load_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->power_max_local_load_division);
}

static ssize_t store_power_max_local_load_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->power_max_local_load_division = val;
	return count;
}
...
static ssize_t show_bump_freq_weight_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->bump_freq_weight_division);
}

static ssize_t store_bump_freq_weight_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->bump_freq_weight_division = val;
	return count;
}

static ssize_t show_power_bump_freq_weight_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->power_bump_freq_weight_division);
}

static ssize_t store_power_bump_freq_weight_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->power_bump_freq_weight_division = val;
	return count;
}

static ssize_t show_power_cpu_load_max_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->power_cpu_load_max_division);
}

static ssize_t store_power_cpu_load_max_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->power_cpu_load_max_division = val;
	return count;
}
...
static ssize_t show_cpu_load_max_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->cpu_load_max_division);
}

static ssize_t store_cpu_load_max_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->cpu_load_max_division = val;
	return count;
}

static ssize_t show_cpu_load_current_division(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%lu\n", tunables->cpu_load_current_division);
}

static ssize_t store_cpu_load_current_division(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->cpu_load_current_division = val;
	return count;
}
...
static ssize_t show_go_hispeed_load_adt_freq_calc(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->go_hispeed_load_adt_freq_calc);
}

static ssize_t store_go_hispeed_load_adt_freq_calc(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
...
static ssize_t show_power_save_cpu_load(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->power_save_cpu_load);
}

static ssize_t store_power_save_cpu_load(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->power_save_cpu_load = val;
	return count;
}

static ssize_t show_power_save_max_local_load(struct cpufreq_gabriel_plus_tunables
		*tunables, char *buf)
{
	return sprintf(buf, "%u\n", tunables->power_save_max_local_load);
}

static ssize_t store_power_save_max_local_load(struct cpufreq_gabriel_plus_tunables
		*tunables, const char *buf, size_t count)
{
	int ret;
	unsigned long val;
	...
	if (ret < 0)
		return ret;
	tunables->power_save_max_local_load = val;
	return count;
}
...
show_store_gov_pol_sys(go_hispeed_load_adt_freq_calc);
show_store_gov_pol_sys(max_local_load);
show_store_gov_pol_sys(max_local_load_division);
show_store_gov_pol_sys(power_max_local_load_division);
...
gov_sys_pol_attr_rw(go_hispeed_load_adt_freq_calc);
gov_sys_pol_attr_rw(max_local_load);
gov_sys_pol_attr_rw(max_local_load_division);
gov_sys_pol_attr_rw(power_max_local_load_division);
...
static struct attribute *gabriel_plus_attributes_gov_sys[] = {
	...
	&go_hispeed_load_adt_freq_calc_gov_sys.attr,
	&max_local_load_gov_sys.attr,
	&max_local_load_division_gov_sys.attr,
	&power_max_local_load_division_gov_sys.attr,
...
static struct attribute *gabriel_plus_attributes_gov_pol[] = {
	...
	&go_hispeed_load_adt_freq_calc_gov_pol.attr,
	&max_local_load_gov_pol.attr,
	&max_local_load_division_gov_pol.attr,
	&power_max_local_load_division_gov_pol.attr,
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_POLICY_INIT:
		...
		if (!tuned_parameters[policy->cpu]) {
			...
			tunables->power_max_local_load_division= DEFAULT_POWER_MAX_LOCAL_LOAD_DIVISION;
